...
| 10 | Wire concat mode into the global sequence cache (`~/.vfam_cache`).  Extend the cache key with a stable hash of `concatenation.proteins` (names, aliases, length_range, locus_tag_hint) when `region == "concatenated"`. | DONE — `cache.marker_set_hash()` + `SequenceCache.{get,store,get_empty,store_empty,download_lock}_concat` + `fetch.load_proteins_from_marker_dir`; pipeline_concat dispatches per-species through the same 3-tier flow as single-protein (negative-cache fast path → lock + cached entry → miss → download + store) |

Phases 8–9 (iterative branch-outlier loop in concat mode + per-family report PDF coverage page) shipped as v1.2.0; phase 10 (concat-mode cache integration) shipped as v1.2.2.
...
6. ~~**Concat-mode cache keying.**~~ **DONE (phase 10).** When `region == "concatenated"`, the cache key extends to include `cache.marker_set_hash(spec)` — a stable 8-char hex hash over each marker's name, aliases, subfamily-aware aliases, length_range, and locus_tag_hint. Cache entries live under `~/.vfam_cache/protein/txid<taxid>_concat_marker<hash8>_<max>/` containing per-marker `<safe_marker>.gb` files plus `manifest.json`. Single-protein cache entries are unaffected — both schemes coexist under `protein/`.
